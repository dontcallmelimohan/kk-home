...
## 实验区页面的地址与相对路径（踩过一次，很隐蔽）
- 页面地址是 `/lab/<slug>`，**结尾没有斜杠**，所以相对路径的解析基准是 `/lab/` 而不是 `/lab/<slug>/`。页里任何相对引用（`style/main.css`、`js/x.js`）都会被浏览器请求成 `/lab/style/main.css` 之类 → **全 404，但页面本身是 200**，看起来就是「能打开但坏了」。
- 搬新 demo 进来时，先看 `href=`/`src=` 里有没有不以 `/`、`http`、`#`、`data:` 开头的。有就改成绝对路径。
- CSS 里的 `@import`/`url()` **不用改** —— 那是按样式表自身地址解析的。
- 冒烟测试里有对应断言（`实验页引用的 N 个地址全部可访问`），它按浏览器规则 `new URL(href, base)` 解析后再真打一遍。**写这类检查时不能只挑以 `/` 开头的引用，否则漏掉的就是这个 bug 本身**。
- **删除一个实验页 = 删 `lib/lab.js` 的 `ITEMS` 条目 + 删对应 html**，两处即可。索引页、首页 Lab 模块、冒烟测试、doctor 全部遍历 `ITEMS`，不要在这些地方写死 slug。
