...
function footer(site) {
  ...
  parts.push(
    '<span><a href="/lab">Lab</a> / <a href="/feed.xml">RSS</a> / <a href="/sitemap.xml">Sitemap</a></span>'
  );
  parts.push('</div>');
  if (site.footerNote) {
...
function sideModules(notes, photos) {
  ...
  // 首页的 Lab 入口。不做独立横幅，而是沿用 Notes / Photos 那套侧栏模块的骨架 ——
  // 实验区是「顺手玩一下」的东西，不该抢正文的位子，但也不该只藏在页脚里等人发现。
  // 具体哪几个上首页由 ITEMS 里的 home 标记决定，免得同一条信息在 lab.js
  // 和这里各写一份，改了一处忘另一处。
  const labItems = lab.ITEMS
    .filter((it) => it.home)
    .map((it) => (
      '<li>' +
      '<strong><a href="/lab/' + esc(it.slug) + '">' + esc(it.title) + '</a></strong>' +
      '<p>' + esc(it.hint) + '</p>' +
      '</li>'
    ))
    .join('\n');
  ...
  return [
    ...
    labItems
      ? [
          '<section aria-label="Lab" class="side-module side-module--lab">',
          '<p class="kicker"><a href="/lab">Lab</a></p>',
          '<ul class="side-module__list">' + labItems + '</ul>',
          '<a class="text-link" href="/lab">全部实验 →</a>',
          '</section>',
        ].join('\n')
      : '',
    '</div>',
...
function labPage(ctx) {
  ...
  const cards = items
    .map((item) =>
      [
        '<li class="lab-card">',
        '<a href="/lab/' + esc(item.slug) + '">',
        '<span class="kicker">' + esc(item.kicker) + '</span>',
        '<h3>' + esc(item.title) + '</h3>',
        '<p>' + esc(item.note) + '</p>',
        '<span class="lab-card__hint">' + esc(item.hint) + '</span>',
        '</a>',
        '</li>',
      ].join('\n')
    )
  ...
  const body = [
    '<section class="home-hero home-hero--compact">',
    '<h1>Lab</h1>',
    '<p>自己写着玩的小东西。跟前面的写作区互不相干，坏了大不了关掉。</p>',
    '</section>',
    '<section class="article-list">',
    '<div class="section-header">',
  ...
  return layout({
    ...
    active: '/lab',
    loggedIn,
    title: 'Lab · ' + site.eyebrow,
    description: '写着玩的小实验：游戏、小工具、和一些只有手机才能玩的东西。',
    canonical: base + '/lab',
    body,
  });
}
